...
class Rfc6238AuthenticationService:
    ...
    @staticmethod
    def generate_code(security_token: bytes, modifier: bytes = None, interval: int = 30):
        b32secret = _apply_modifier(security_token, modifier)
        totp = pyotp.TOTP(b32secret, interval=interval)
        return totp.now()

    @staticmethod
    def validate_code(security_token: bytes, code: str, modifier: bytes = None, interval: int = 30):
        b32secret = _apply_modifier(security_token, modifier)
        totp = pyotp.TOTP(b32secret, interval=interval)
